...
def agent_create(num_agents):
    ...
    for _ in range(num_agents):
        ...
        colour = species[random.randint(0, len(species)-1)]
        ...
        agent = [start_pos, particle_bearing, velocity_vec, agent_speed, colour]
...
def brain(position, angle, pixels, colour):
    scent_strengths = []
    ...
    for probe_angle in angles:
        ...
        if probe_position[0] > resolution[0] - 1 or probe_position[0] < 0 or probe_position[1] > resolution[1] - 1 or probe_position[1] < 0:
            score = -10000
...
def agent_update(agent, pixels):
    ...
    position, angle, velocity, speed, colour = agent
    #update agent code
