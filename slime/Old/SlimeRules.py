...
def checkIntensity(position, vel, pixels):
    coordsAhead = (round(position[0] + 4*vel[0]), round(position[1] + 4*vel[1]), 0)
    coordsRight = (round(position[0] + 4*vel[0]), round(position[1] + 4*vel[1]), 0)

    
    intensity = pixels[coordsAhead]
    rangedIntensity = 255 / intensity
    return rangedIntensity
...
'''
def updateBalls(balls, pixels):
    index = -1
    for ball in balls:
        ballC, position, velC = ball

        aheadIntensity = checkIntensity(ballC, velC, pixels)
        rightIntensity = checkRightIntensity
'''
...
while True:
    ...
    if useCuda:
        ...
        #balls = updateBalls(balls, pixels)
